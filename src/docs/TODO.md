* Make macro that "marks" all labels (label stuff interceptor thingie).
  I also need to mark labels created with the `label` instruction (`label` is a
  CALM instruction directive that one can create labels with).
  With this system, I could:
    1. Distinguish labels from other symbols (constants, equates, register
    names, etc.)
    2. Enforce syntax rules like "branch instructions should take labels without
       .w/.l suffixes"
    3. Better error messages - "expected a label here" instead of
       "expected abs.w"
    4. Handle the semantic difference between:
        - A label used in a branch (relative addressing)
        - A label used as an absolute address
        - An immediate value or constant
  But currently I don't even remember if I even strictly need any of that.
...
* I now support `$7FFF(a2)` but I should also support `($7FFF,a2)` and add a lot
      ...
      Take a look at these commented out stuff in generate_tests.ts:
      ```
...
* Crap, maybe it's possible to write `(d8,Xn,PC)`
  ...
  support that...
  `(d8,Xn,PC)` is standard Motorola order and other permutations are
  non-standard which I "probably" don't need to support.
